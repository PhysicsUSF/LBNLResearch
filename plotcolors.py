...
N_DAYS = 2.0  # valid day range for interpolation, for use in 11fe reddening fits
...
def lsq_11fe_color_fit(SN, EBV, rv_guess, zp, N_DAYS):
    ...
    def lsq_func(Y):
        ...
        for f in filters:
            ...
            valid[f] = find_valid([t[0] for t in sn11fe], phases, N_DAYS)
            valid_phases = np.array(phases)[valid[f]]
...
def main():
    ...
    lsq_out, valid_phases = lsq_11fe_color_fit(sn14j_dict, EBV_14J, RV_14J, zp, N_DAYS)
    BEST_RV = lsq_out[0][0]
    ...
    for FILTER in FILTERS:
        ...
        filtered_fmr = calc_colors(FILTER, sn2011fe_int, sn2011fe_vmags, zp)
        sn11fe_phases = np.array([t[0] for t in filtered_fmr])
        sn11fe_colors = np.array([t[1] for t in filtered_fmr])

        p1, = plt.plot(sn11fe_phases, sn11fe_colors, 'ro--', mfc='none', mec='r', ms=7)

        valid = valid_phases[FILTER]
        plt.plot(sn11fe_phases[valid],
                 sn11fe_colors[valid],
                 'ro', mfc='r', mec='r', ms=7, alpha=SN11FE_PLOT_ALPHA)
    ...
    fig1.legend([sn11fe_plot_proxy, p2],
    ...
    for f in FILTERS:
        ...
        sn12cu_dict[f] = filter(lambda t: -5<t[0]<35, zip(sn12cu_phases, sn12cu_bandcolors))
    
    lsq_out, valid_phases = lsq_11fe_color_fit(sn12cu_dict, EBV_12CU, RV_12CU, zp, N_DAYS)
    BEST_RV = lsq_out[0][0]
    ...
    fig2.legend([sn11fe_plot_proxy, p2],
